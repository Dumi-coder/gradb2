        ...
        <section class="edit-form-section">
            ...
            <form method="POST" enctype="multipart/form-data" data-password-modal="true">
                ...
                <div class="change-password-modal" aria-hidden="true" data-has-password-errors="<?= $hasPasswordErrors ? '1' : '0' ?>">
                    <div class="change-password-modal-content" role="dialog" aria-modal="true" aria-labelledby="adminChangePasswordTitle">
                        <div class="change-password-modal-header">
                            <h3 class="change-password-modal-title" id="adminChangePasswordTitle">Change Password</h3>
                            <button type="button" class="btn btn-outline js-close-password-modal">Close</button>
                        </div>
                        <div class="change-password-modal-body">
                            <div class="form-group">
                                <label for="admin_password_current" class="form-label">Current Password</label>
                                <input type="password" id="admin_password_current" name="password_current" class="form-input" autocomplete="current-password">
                                <?php if (isset($errors['password_current'])): ?>
                                    <div class="error-message"><?= esc($errors['password_current']) ?></div>
                                <?php endif; ?>
                            </div>
                            <div class="form-group">
                                <label for="admin_password_new" class="form-label">New Password</label>
                                <input type="password" id="admin_password_new" name="password_new" class="form-input" autocomplete="new-password">
                                <?php if (isset($errors['password_new'])): ?>
                                    <div class="error-message"><?= esc($errors['password_new']) ?></div>
                                <?php endif; ?>
                                <div class="js-password-strength-container" id="admin-password-strength-widget"></div>
                            </div>
                            <div class="form-group">
                                <label for="admin_password_confirm" class="form-label">Confirm New Password</label>
                                <input type="password" id="admin_password_confirm" name="password_confirm" class="form-input" autocomplete="new-password">
...
<form id="deletePhotoForm" action="<?= ROOT ?>/admin/profile?action=delete_photo" method="POST" class="profile-delete-form-hidden">

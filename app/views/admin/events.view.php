...
if (!function_exists('renderAdminEventCard')) {
  function renderAdminEventCard($event, $registeredMap, $showRegistration = true, $showModeration = true)
  {
    $eventId = (int)($event['event_id'] ?? 0);
    ...
    <main class="main-content">
        <section class="dashboard-section hosting-section">
            <div class="section-header">
                <h2 class="section-title">Faculty Event Moderation</h2>
        ...
        <div class="events-container">
          <?php if (!empty($hostingEvents)): ?>
            <?php foreach ($hostingEvents as $event): ?>
              <?php renderAdminEventCard($event, $registeredMap, false, true); ?>
            <?php endforeach; ?>
          <?php else: ?>
        ...
        <div class="events-container">
          <?php if (!empty($attendingEvents)): ?>
            <?php foreach ($attendingEvents as $event): ?>
              <?php renderAdminEventCard($event, $registeredMap, true, false); ?>
            <?php endforeach; ?>
          <?php else: ?>
      ...
      <section class="dashboard-section moderation-section">
        <div class="section-header">
          <h2 class="section-title">All Faculty Events</h2>
            </div>

            <div class="events-container">
                <?php if (!empty($events)): ?>
                    <?php foreach ($events as $event): ?>
              <?php renderAdminEventCard($event, $registeredMap, true, true); ?>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div class="empty-state">No active events found for your faculty.</div>
                <?php endif; ?>
            </div>
        </section>
    </main>
...
(function() {
  ...
  document.querySelectorAll('.register-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      postAction('/admin/Events/register', eventId, () => location.reload());
    });
  });
  ...
  document.querySelectorAll('.unregister-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const eventId = btn.getAttribute('data-event-id');
      postAction('/admin/Events/unregister', eventId, () => location.reload());
    });
  });
  ...
  document.querySelectorAll('.toggle-reg-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const eventId = btn.getAttribute('data-event-id');
      postAction('/admin/Events/toggleRegistration', eventId, () => location.reload());
    });
  });
  ...
  document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const eventId = btn.getAttribute('data-event-id');
      postAction('/admin/Events/delete', eventId, () => location.reload());
    });
  });
  ...
  document.querySelectorAll('.details-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const eventId = btn.getAttribute('data-event-id');
      fetch(root + '/admin/Events/getEvent?id=' + encodeURIComponent(eventId), {

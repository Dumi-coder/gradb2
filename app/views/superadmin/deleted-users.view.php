        ...
        <section class="dashboard-section">
                ...
                <div class="filters-container">
                    ...
                    <div class="filter-options-enhanced">
                        <div class="filter-item">
                            ...
                            <select id="faculty-filter" class="filter-select-enhanced">
                                <option value="">All Faculties</option>
                                <?php foreach ($faculties as $faculty): ?>
                                <option value="<?= $faculty['faculty_id'] ?>"><?= esc($faculty['faculty_name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="filter-item">
            ...
            <div class="users-container" id="deleted-users-container">
                ...
                <div class="empty-state">
                    <i class="fas fa-user-check sae-24"></i>
                    <h3>No Deleted Users</h3>
                    <p>There are no deleted user accounts in the system.</p>
                </div>
                <?php else: ?>
                <?php foreach ($deletedUsers as $user): ?>
                <div class="user-card deleted-user-card" data-role="<?= $user['role'] ?>" data-name="<?= strtolower($user['name']) ?>" data-date="<?= $user['deleted_date'] ?>" data-faculty="<?= $user['faculty_id'] ?>">
...
<div id="recoveryModal" class="custom-modal sae-10">
    <div class="modal-content">
        <div class="modal-icon modal-icon-success">
            <i class="fas fa-undo"></i>
        </div>
        <h3 class="sae-25">Recover User Account</h3>
        <p id="recovery-message" class="sae-26">Are you sure you want to recover this account?</p>
...
<div id="alertModal" class="custom-modal sae-10">
    <div class="modal-content">
        <div class="modal-icon" id="alert-icon">
            <i class="fas fa-check"></i>
        </div>
        <h3 id="alert-title" class="sae-25">Success</h3>
        <p id="alert-message" class="sae-26"></p>
        <div class="modal-buttons">
...
function filterUsers(searchTerm, facultyFilter, roleFilter) {
    ...
    userCards.forEach(card => {
        ...
        if (matchesSearch && matchesFaculty && matchesRole) {
...
async function confirmRecovery() {
    ...
    try {
        ...
        const response = await fetch('<?=ROOT?>/superadmin/DeletedUsers', {
            method: 'POST',
            body: formData
        });

...
<head>
    ...
    <link rel="stylesheet" href="<?=ROOT?>/assets/css/faculty-admin-extracted.css">
            ...
            <div class="header-content">
                <div class="welcome-section">
                    ...
                    <?php else: ?>
                        ...
                        <?php
                        $headerFacultyName = $_SESSION['faculty_name'] ?? null;

                        if (empty($headerFacultyName) && !empty($_SESSION['user_id'])) {
                            $facultyAdminModel = new FacultyAdmin();
                            $activeAdmin = $facultyAdminModel->getActiveAdminByUserId($_SESSION['user_id']);

                            if ($activeAdmin && !empty($activeAdmin->faculty_id)) {
                                $_SESSION['faculty_id'] = $activeAdmin->faculty_id;

                                $facultyModel = new Faculty();
                                $facultyRecord = $facultyModel->first(['faculty_id' => $activeAdmin->faculty_id]);

                                if ($facultyRecord && !empty($facultyRecord->faculty_name)) {
                                    $headerFacultyName = $facultyRecord->faculty_name;
                                    $_SESSION['faculty_name'] = $headerFacultyName;
                                }
                            }
                        }
                        ?>
                        <p class="student-role">Faculty Admin • Faculty: <?= esc($headerFacultyName ?: 'N/A') ?></p>
                    <?php endif; ?>
                </div>

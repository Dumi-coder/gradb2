...
<div class="dashboard-container">
  ...
  <main class="main-content">
    <section class="dashboard-section counsellor-edit-page">
      <div class="edit-form-section">
        <div class="counsellor-edit-top">
        ...
        <form method="POST" enctype="multipart/form-data" data-password-modal="true">
          <div class="profile-picture-section counsellor-edit-picture">
            <div class="profile-picture-preview counsellor-avatar-preview">
              <?php if (!empty($user->profile_photo_url)): ?>
                <img src="<?= esc($user->profile_photo_url) ?>" alt="Profile Picture" id="profilePreview">
              <?php else: ?>
                <span id="profileInitials" class="counsellor-avatar-initial"><?= strtoupper(substr((string)($user->name ?? 'C'), 0, 2)) ?></span>
              <?php endif; ?>
            </div>

            <div class="counsellor-picture-actions">
          ...
          <div class="change-password-modal" aria-hidden="true" data-has-password-errors="<?= $hasPasswordErrors ? '1' : '0' ?>">
            <div class="change-password-modal-content" role="dialog" aria-modal="true" aria-labelledby="counsellorChangePasswordTitle">
              <div class="change-password-modal-header">
                <h3 class="change-password-modal-title" id="counsellorChangePasswordTitle">Change Password</h3>
                <button type="button" class="btn btn-outline js-close-password-modal">Close</button>
              </div>
              <div class="change-password-modal-body">
                <p class="counsellor-password-modal-subtitle">Use a strong password to keep your counsellor account secure.</p>

                <div class="form-group counsellor-password-group">
                  <label class="form-label" for="counsellor_password_current">Current Password</label>
                  <div class="counsellor-password-input-wrap">
                    <input
                      type="password"
                      id="counsellor_password_current"
                      name="password_current"
                      class="form-input counsellor-password-input"
                      placeholder="Enter current password"
                      autocomplete="current-password"
                    >
                    <button type="button" class="counsellor-password-toggle" data-password-toggle data-target="counsellor_password_current" aria-label="Show current password">
                ...
                <div class="form-group counsellor-password-group">
                  <label class="form-label" for="counsellor_password_new">New Password</label>
                  <div class="counsellor-password-input-wrap">
                    <input
                      type="password"
                      id="counsellor_password_new"
                      name="password_new"
                      class="form-input counsellor-password-input"
                      placeholder="Enter new password"
                      autocomplete="new-password"
                    >
                    <button type="button" class="counsellor-password-toggle" data-password-toggle data-target="counsellor_password_new" aria-label="Show new password">
                      <i class="fas fa-eye"></i>
                    </button>
                  </div>
                  <div class="js-password-strength-container" id="counsellor-password-strength-widget"></div>
                </div>

                <div class="form-group counsellor-password-group">
                  <label class="form-label" for="counsellor_password_confirm">Confirm New Password</label>
                  <div class="counsellor-password-input-wrap">
                    <input
                      type="password"
                      id="counsellor_password_confirm"
                      name="password_confirm"
                      class="form-input counsellor-password-input"
                      placeholder="Re-enter new password"
                      autocomplete="new-password"
                    >
                    <button type="button" class="counsellor-password-toggle" data-password-toggle data-target="counsellor_password_confirm" aria-label="Show confirm password">
                      <i class="fas fa-eye"></i>
                    </button>
                  </div>
                  <small class="form-help counsellor-password-match" id="counsellorPasswordMatchHint" aria-live="polite"></small>
                </div>

                <div class="counsellor-password-tips" aria-hidden="true">
                  <p class="counsellor-password-tips-title">Password tips</p>
                  <ul>
...
<script>
  function previewImage(input) {
    ...
    reader.onload = function (e) {
      if (preview) {
        ...
        const previewContainer = document.querySelector('.counsellor-avatar-preview');
        if (!previewContainer) {
          return;
        }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const newPasswordField = document.getElementById('counsellor_password_new');
    const confirmPasswordField = document.getElementById('counsellor_password_confirm');
    const matchHint = document.getElementById('counsellorPasswordMatchHint');

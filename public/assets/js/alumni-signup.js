/**
 * Alumni Signup Form Handler with AJAX
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!signupForm) {
        console.error('Alumni signup form not found!');
        return;
    }
    
    console.log('Alumni signup form handler initialized');
    ...
    signupForm.addEventListener('submit', function(e) {
        ...
        // Construct the URL - use current path and ensure it points to alumni/Auth
        let formAction = window.location.pathname;
        
        // If we're not already on the Auth page, construct the URL
        if (!formAction.includes('/alumni/Auth') && !formAction.includes('/Alumni/Auth')) {
            // Remove any trailing parts and add /alumni/Auth
            const basePath = formAction.split('/').slice(0, -1).join('/');
            formAction = basePath + '/alumni/Auth';
        }
